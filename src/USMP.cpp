...
USMPClient::USMPClient(const char *psk)
    : _psk(psk), _initialized(false), _backoff_ms(2000), _last_attempt_ms(0),
      _on_connect(nullptr), _on_disconnect(nullptr), _on_reconnect(nullptr),
      _on_message(nullptr) {
...
String USMPClient::deviceId() {
  char buf[18];
  snprintf(buf, sizeof(buf), "%02x:%02x:%02x:%02x:%02x:%02x", _ctx.device_id[0],
           _ctx.device_id[1], _ctx.device_id[2], _ctx.device_id[3],
           _ctx.device_id[4], _ctx.device_id[5]);
  return String(buf);
}
...
void USMPClient::maintain() {
  ...
  if (!_ctx.established) {
    uint32_t now = millis();
    if (now - _last_attempt_ms < _backoff_ms)
      return;
...
void USMPClient::onMessage(void (*cb)(const uint8_t *, size_t len)) {
  _on_message = cb;
}
